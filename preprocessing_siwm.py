...
def mtcnn_caller(path, image_folder,image):
    ...
    img_cropped = mtcnn(img, save_path=path+'spoof/'+image_folder+image)
    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType._2D, flip_input=False)
    input = io.imread(path+'spoof/'+image_folder+image)
    image = image.strip('.jpg')
    preds = fa.get_landmarks(input)
    numpy.save(path+'spoof/'+image_folder+image, preds)
